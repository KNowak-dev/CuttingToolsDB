...
class AddCoatingToTool:
    ...
    def add(self, edv: EDVNumber, coating_factory: str):
        tool = self.tool_storage.get(edv)
        if tool is None:
            raise KeyError(f"Tool with EDV {edv} not found")

        coating = self.coating_storage.get(coating_factory)
        if coating is None:
            raise KeyError(f"Coating with factory {coating_factory} not found")
